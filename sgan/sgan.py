...
class SGAN:
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        # f.close()
        size=str(self.img_rows)
        f = np.load('E:/UnderWaterDataset/mergeData_'+size+'.npz')
        X_train, y_train = f['arr_0.npy'], f['arr_1.npy']
        f.close()
        ...
        dis_weight = './saved_model/'+size+'_dis_model.hdf5'
        gen_weight = './saved_model/'+size+'_gen_model.hdf5'
    ...
    def sample_images(self, epoch,size):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig('images_'+size+'/mnist_%d.png' % epoch)
        plt.close()
...
if __name__ == '__main__':
    sgan = SGAN()
    sgan.train(epochs=1001, batch_size=32, sample_interval=50)
